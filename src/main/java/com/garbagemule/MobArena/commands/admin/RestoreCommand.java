...
public class RestoreCommand implements Command
{
    ...
    public boolean execute(ArenaMaster am, CommandSender sender, String... args) {
        ...
        if (am.getArenaWithPlayer(player) != null) {
            am.sendMessage(sender, "Player is currently in an arena.");
            return true;
        }

        if (InventoryManager.restoreFromFile(am.getPlugin(), player)) {
            am.sendMessage(sender, "Restored " + args[0] + "'s inventory!");
        } else {
            am.sendMessage(sender, "Failed to restore " + args[0] + "'s inventory.");
        }
        return true;
    }
